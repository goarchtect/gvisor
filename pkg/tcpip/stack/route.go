...
//
// Returns ErrWouldBlock if address resolution requires blocking (e.g. wait for
// ARP reply). If doneCh is provided, the channel will receive a link address if
// address resolution is successful, otherwise it will be closed. If waker is
// provided, it will be notified when address resolution is complete, regardless
