...
// | Incomplete |            | Max probes sent without reply              |          | Notify     | Removed |
...
// | Probe      |            | Max probes sent without reply              |          | Notify     | Removed |
...
func TestEntryIncompleteGetsDeleted(t *testing.T) {
	c := DefaultNUDConfigurations()
	c.MaxMulticastProbes = 3
	e, nudDisp, linkRes, clock := entryTestSetup(c)
...
func TestEntryProbeGetsDeleted(t *testing.T) {
	c := DefaultNUDConfigurations()
	c.MaxMulticastProbes = 3
	...
	// Wait for the last probe to expire, causing the entry to be removed.
	clock.Advance(c.RetransmitTimer)
	wantEvents := []testEntryEventInfo{
		{
			EventType: entryTestAdded,
