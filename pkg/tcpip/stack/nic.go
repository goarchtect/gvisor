...
func (n *NIC) WritePacket(r *Route, gso *GSO, protocol tcpip.NetworkProtocolNumber, pkt *PacketBuffer) *tcpip.Error {
	...
	// Buffer the done channel to save the result of the address resolution in
	// case resolution succeeds before anyone starts listening on the channel.
	doneCh := make(chan tcpip.LinkAddress, 1)
	if err := r.Resolve(doneCh, nil); err != nil {
		if err == tcpip.ErrWouldBlock {
			r := r.Clone()
			n.stack.linkResQueue.enqueue(doneCh, &r, protocol, pkt)
			return nil
		}
		return err
	}
