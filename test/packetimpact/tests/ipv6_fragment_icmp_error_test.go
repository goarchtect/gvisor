...
func TestIPv6ICMPEchoRequestFragmentReassembly(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			fragments, _ := createFragmentedICMPEchoRequest(t, conn, test.firstPayloadLength, test.payload, test.secondFragmentOffset)
			...
			if !bytes.Equal(icmpPayload, wantPayload) {
				t.Fatalf("got unexpected payload = %s, want = %s",
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectErrorDetail.typeSpecificIsUsed {
				...
				if gotTypeSpecific != wantTypeSpecific {
					t.Fatalf("got unexpected type specific value = %s, want = %s", gotTypeSpecific, wantTypeSpecific)
				}
			}
